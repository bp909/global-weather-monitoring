...
%     GB(:,:,i) = CloudSepLMANaN(CHAN_B,lmachan,thresh_BL,thresh_BS);
    GR(:,:,i) = CHAN_R;%(1876:2476,1427:2027);
    GG(:,:,i) = CHAN_G;%(1876:2476,1427:2027);
    GB(:,:,i) = CHAN_B;%(1876:2476,1427:2027);
end



figure;
histogram(GR(2126,1443,:))
figure;
histogram(GG(2126,1443,:))
figure;
histogram(GB(2126,1443,:))
...
% for j = 1:imagesiz_X
%     for k = 1:imagesiz_Y
%         threshmean_R(j,k) = min(nonzeros(GR(j,k,:)));
%         threshmean_G(j,k) = min(nonzeros(GG(j,k,:)));
%         threshmean_B(j,k) = min(nonzeros(GB(j,k,:)));
%     end
% end
...
GG(GG == 0) = NaN;
GB(GB == 0) = NaN;
threshmean_R = min(GR,[],3,'omitnan');
threshmean_G = min(GG,[],3,'omitnan');
threshmean_B = min(GB,[],3,'omitnan');
figure;
...
figure;
imshow(res_RGB)
imwrite(res_RGB, 'pivelminim_RGB.png')
